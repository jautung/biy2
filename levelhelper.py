...
def calculate_cell_size(window: Window, map: Map):
    max_cell_size_horizontal = (
        window.width - 2 * MINIMUM_WINDOW_PADDING_SIZE
    ) / map.number_columns
    max_cell_size_vertical = (
        window.height - 2 * MINIMUM_WINDOW_PADDING_SIZE
    ) / map.number_rows
...
def calculate_asset_position(
    ...
    cell_size: int,
    asset_size: int,
    window_padding_horizontal: int,
    window_padding_vertical: int,
):
    x = (
        window_padding_horizontal
        + column_index * cell_size
        + (cell_size - asset_size) / 2
    )
