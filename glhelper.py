...
def reset_display(window: Window):
    ...
    run_block_in_matrix_mode(
        mode=GL_MODELVIEW,
        block_func=clear_modelview_mode,
    )
    
    def clear_projection_mode():
        glLoadIdentity()
        glOrtho(
    ...
    run_block_in_matrix_mode(
        mode=GL_PROJECTION,
        block_func=clear_projection_mode,
    )
    
    def reset_modelview_mode():
        glLoadIdentity()
    run_block_in_matrix_mode(
        mode=GL_MODELVIEW,
        block_func=reset_modelview_mode,
    )
